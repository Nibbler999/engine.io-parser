...
exports.encodePacket = function (packet, supportsBinary, utf8encode, callback) {
  ...
  if (packet.data && packet.data._fragments) {
    return encodeFragments(packet, supportsBinary, callback);
...
function encodeFragments(packet, supportsBinary, callback) {
  if (!supportsBinary) {
    packet.data = Buffer.concat(packet.data._fragments);
    return exports.encodeBase64Packet(packet, callback);
  }

  var typeBuffer = new Buffer(1);
  typeBuffer[0] = packets[packet.type];
  packet.data._fragments.unshift(typeBuffer);
  return callback(packet.data._fragments);
}
...
exports.decodePacket = function (data, binaryType, utf8decode) {
  ...
  if (Array.isArray(data)) {
   ...
   return { type: packetslist[type], data: { _fragments: data } };
  }
...
function encodeOneBinaryPacket(p, doneCallback) {
  ...
  function onBinaryPacketEncode(packet) {
    ...
    var sizeBuffer, encodingLength;

    if (typeof packet === 'string') {
      encodingLength = '' + packet.length;
